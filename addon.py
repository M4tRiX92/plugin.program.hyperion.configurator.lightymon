...
settings_cache_path = "/storage/.kodi/userdata/addon_data/plugin.program.hyperion.configurator.lightymon/settings.xml"
default_config_path="/storage/.config/hyperion.config.json"
run_command="/storage/hyperion/bin/hyperiond.sh /storage/.kodi/addons/plugin.program.hyperion.configurator.lightymon-master/hyperion.config.new"
gpio_version=False
...
def myfunction(device = None, ledh = None, ledv = None, direction = None, center_corner = None):
    ...
    if center_corner is None:
        ...
        #webseite
        xbmc.log("Lightymon: Starting...", level=xbmc.LOGNOTICE)
    ...
    try:
        if center_corner is None:
            ...
            if updater.isUpdateAvailable():
                updater.installUpdate()
                xbmc.log("Lightymon: Update installed", level=xbmc.LOGNOTICE)
    ...
    try:
        if HyperPyCon.HyperPyCon.amIonOSMC():
            settings_cache_path = "/home/osmc/.kodi/userdata/addon_data/plugin.program.hyperion.configurator.lightymon/settings.xml"
            ...
            run_command="hyperiond /home/osmc/.kodi/addons/plugin.program.hyperion.configurator.lightymon-master/hyperion.config.new"
    ...
    try:
        ...
        if selected_device == 2 or selected_device == 3:
            if center_corner is None:
                ...
                #webseite
                xbmc.log("Lightymon:" + "We have detected that your system does not have spi enabled. You can still continue, but leds may not work if you're using GPIO/SPI connection", level=xbmc.LOGNOTICE)
            gpio_version=True
    ...
    # if xbmcgui.Dialog().yesno(addonname, "Would you like to download recommended settings for the Lightymon you " +
        #                                     "selected? WARNING: that will overwrite your current addon settings!"):
        #   try:
        #      settingsxml = urllib2.urlopen("http://img.lightymon.com/download/settings.xml-"+suffix).read()
        ...
        if gpio_version or device_versions[selected_device] == HyperPyCon.HyperPyCon.lightymonXL:
            # turn off unused leds if this is GPIO version of Lightymon
        ...
        if center_corner is None:
            ...
            #webseite
            xbmc.log("Lightymon: That's all! Now we will attempt to restart hyperion...", level=xbmc.LOGNOTICE)
        ...
        if center_corner is None:
           ...
           #webseite
            xbmc.log("Lightymon: That's all! Enjoy", level=xbmc.LOGNOTICE)
